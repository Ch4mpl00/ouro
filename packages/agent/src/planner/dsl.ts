...
// Plan DSL — the language the planner LLM emits and the executor runs.
//
...
// plan, but a step only sees the bindings it names. This is the
...
// For Phase 1 we treat plan validation as a server-side Zod check;
...
export interface PlanSchemaDeps {
...
export interface PlanSchemaBundle {
  ...
  planToJsonSchema: () => unknown;
}

export function createPlanSchema(deps: PlanSchemaDeps): PlanSchemaBundle {
  if (deps.knownTools.length === 0) {
    throw new Error("createPlanSchema: knownTools must be non-empty");
  }
  if (deps.knownSkills.length === 0) {
    throw new Error("createPlanSchema: knownSkills must be non-empty");
  }
  ...
  // The check lives in postCheckPlan() instead and runs after the
  ...
  // both to keep traces readable and to keep the planner from
...
export function parsePlan(
  ...
  if (!result.success) {
    return { ok: false, errors: formatPlanErrors(result.error) };
  }
  const plan = result.data as Plan;
  const semanticErrors = postCheckPlan(plan);
...
// either/or between two optional fields). Walks the plan tree and
// returns one error string per violation, formatted with the same
// `at steps[N]...` convention as formatPlanErrors().
function postCheckPlan(plan: Plan): string[] {
  const errors: string[] = [];
  ...
  plan.steps.forEach((s, i) => visit(s, `steps[${i}]`));
  return errors;
}

// Render Zod issues as one-line human-readable strings suitable for
// feeding back to the planner LLM in a retry. Raw Zod issue trees are
...
// success.
export function formatPlanErrors(error: z.ZodError): string[] {
  const out: string[] = [];
  for (const issue of error.issues) {
    const path = renderPath(issue.path);
    const where = path ? `at ${path}` : "at plan root";
